...
library(tidyverse)
weather_tpa <- read_csv("https://raw.githubusercontent.com/Dhrish96/dataviz_final_project/main/data/tpa_weather_16_17.csv")
# random sample 
...
head(comments_data, 6)
```

```{r}

comments_data_words <- comments_data %>% 
  unnest_tokens(word, comments) %>% 
  mutate(
    course = str_trunc(course, 
                     width = 15)) %>% 
  select(word, course) %>% 
  head()
```
